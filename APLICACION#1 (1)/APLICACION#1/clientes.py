...
class CClientes:
    ...
    def modificarClientes(nombres, apellidos, telefono, correo, direccion, sexo, rut):

        try:
            cone=CConexion.ConexionBaseDeDatos()
            cursor = cone.cursor()
            sql = "UPDATE usuarios SET usuarios.nombres = %s, usuarios.apellidos = %s, usuarios.telefono = %s, usuarios.correo = %s, usuarios.direccion = %s, usuarios.sexo = %s WHERE usuarios.rut = %s;"
            valores = (rut, nombres, apellidos, telefono, correo, direccion, sexo)
            cursor.execute(sql, valores)
            cone.commit()
